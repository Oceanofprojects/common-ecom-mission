...
$pay_details = $payMdl->get_payment_info('pay_OVRw0TNL55oELX');

if (!isset($_GET['invoice_id'])) {
    die('Invaild invoice ID, Please check.');
}
$productList = $prdObj->getOrderDetailById($_GET['invoice_id']);
if(count($productList['data'])>0){
    $productList = $productList['data'];
}else{
    die('Zero product list for this invoice ID, Please check.');
}
...
$courier = $pay_details->notes->notes_key_2->courier;
$cc_price = $pay_details->notes->notes_key_2->cc_price;
$subtotal = $pay_details->notes->notes_key_2->product_price;
$total = $pay_details->notes->notes_key_2->payment;
$online_fee = $pay_details->notes->notes_key_2->online_payment_charge;
$o_off = $pay_details->notes->notes_key_2->off_price;
...
for ($i = 0; $i < count($list); $i++) {
    ...
    if ($productInfo['status']) {
        ...
        <td><br><br>" . (($productInfo['data'][0]['price']-$productInfo['data'][0]['price']*$productInfo['data'][0]['offer']/100) * $list[$i]['qnty']) . "<br></td>
...
<h1></h1>
<h4>Date : {$date}</h4>
<h4>Order Status : {$order_status}, $courier($courier_order_number)</h4>
...
<tr>
   <td colspan="4" ></td>
    <td colspan="2" style="text-align:right;">Total Price : {$subtotal}rs</td>
</tr>
 <tr>
    <td colspan="7" ></td>
    </tr>
...
<tr>
   <td colspan="4" ></td>
    <td colspan="2" style="text-align:right;">Online fee(0.5%) : {$online_fee}rs</td>
</tr>
    <tr>
