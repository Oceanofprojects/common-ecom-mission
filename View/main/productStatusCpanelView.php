            ...
            <table id="jquery-datatable-example-no-configuration" class="display" style="width:100vw">
                ...
                if($data['data'] !==0){
                    ...
                    for($i=0;$i<count($res);$i++){
                        echo "<tr><td><a href='index.php?key=1037d9ea3af16d70f0ce197f737e4ca6a3d1f436ce0365689334069ea9772565&cid=".base64_encode($res[$i]['cid'])."&controller=admin'>".$res[$i]['cid']."</a></td><td>".$res[$i]['name']."</td><td><a href='Invoice?invoice_id=".$res[$i]['order_id']."'>".$res[$i]['order_id']."</a></td><td><!-- <a href=\"pay/orderdetail/?id=".$res[$i]['pay_proof']."\">Payment proof</a>-->-</td><td>".$res[$i]['cart_date']."</td><td>".getStatusDP($res[$i]['status'],"'".$res[$i]['order_id']."'")."</td></tr>";
                    }
                }else{
