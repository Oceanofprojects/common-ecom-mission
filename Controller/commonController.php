...
class commonController
{
    ...
    public function autoValidate($data, $validateArgs)
    {
        if (count($validateArgs)) {
            ...
            if ($vaildateResponse[0]['status']) {
                return $this->validateResults;//OVERALL VALIDATION ARRAYS
            } else {
    ...
    public function validate($data, $validateArgs)
    {
        for ($j = 0; $j < count($validateArgs); $j++) {
            // var_dump($data[$validateArgs[$j][0]]);exit;
            /*VALIDATE ARGS
            ...
            if ($validateArgs[$j][1] == 'checkMinLength') {
                ...
                if (preg_match('@[0-9]@',str_replace(' ', '', $data[$validateArgs[$j][0]])) || preg_match('@[^\w]@', str_replace(' ', '', $data[$validateArgs[$j][0]])) || preg_match('@[_]@', str_replace(' ', '', $data[$validateArgs[$j][0]]))) {
    ...
    public function removeUniqueWordForLabel($validateArrs, $arrData)
    {
        ...
        switch ($arrData) {
            ...
            case 'c_name':
                ...
                break;
            case 'ph_num':
                $pre_fix = 'Phone number';
                break;
            case 'whatsapp_num':
                $pre_fix = 'Whatsapp number';
                break;
            case 'address_1':
            case 'address_2':
                $pre_fix = 'Address';
