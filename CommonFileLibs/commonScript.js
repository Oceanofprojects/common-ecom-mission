...
function sendData(displayMsg,form,callback){
    ...
    if(reqUrlStatus && reqMethodStatus){
        return $.ajax({
            url : reqUrl+''+debugArgs,
            type : reqType,
            data:$(form).serialize(),
            success: function (e) {                
                res = JSON.parse(e);
                responseHandler(displayMsg,res);
            }
        }).done(callback);
    }else{
...
function anumAJAX(desUrl,redirectUrl) {
    if(desUrl.trim().length == 0){
        alert("DestinationURI must to send AJAX action!")
    }else{
